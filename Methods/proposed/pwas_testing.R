...
opt = data.frame(
  g = "/proj/yunligrp/users/chanhwa/pwas/whi_pwas_freeze9/result/cardio2_ACE2/proposed_pval_0.000005/test.cardio2_ACE2.dose.txt",
  p = "/proj/yunligrp/users/chanhwa/pwas/whi_pwas_freeze9/result/cardio2_ACE2/proposed_pval_0.000005/test.protein_level.txt",
  b = "/proj/yunligrp/users/chanhwa/pwas/whi_pwas_freeze9/result/cardio2_ACE2/proposed_pval_0.000005/train.cardio2_ACE2.betas.EN.txt",
  l = "/proj/yunligrp/users/chanhwa/pwas/whi_pwas_freeze9/result/cardio2_ACE2/proposed_pval_0.000005/train.cardio2_ACE2.log",
  o = "/proj/yunligrp/users/chanhwa/pwas/whi_pwas_freeze9/result/cardio2_ACE2/proposed_pval_0.000005/example.test.protein"
)
...
if (is.na(opt$b)) {
  ...
  cat("It might be the case that the trained model reports less than 2 non-zero coefficients,\n")
  cat("so no reason to do testing\n")
...
# Let colnames of X.test as CHR:POS:REF:ALT
...
if (info$rsq < 0.01){
  cat("This prediction model is not very good in terms of p-value or adjusted r-squared, so we won't use it for predicting protein level.\n", file = paste0(opt$o, ".log"), append = T)
  cat("This prediction model is not very good in terms of p-value or adjusted r-squared, so we won't use it for predicting protein level.\n")
  quit("no")
}
